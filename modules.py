...
class retina(object):
    ...
    def foveate(self, x, l):
        ...
        for i in range(self.k):
            # extract patchs according to location center l and patch_size
            # every image has only k patch
        ...
        phi = torch.cat(phi, 1)
        phi = phi.view(phi.shape[0], -1) # [32,64] flatten 8*8 patch into 64-dim vector
    ...
    def extract_patch(self, x, l, size):
        ...
        for i in range(B):
            ...
            # and finally extract
            patch.append(im[:, :, from_y:to_y, from_x:to_x]) # [1,1,8,8]
...
class glimpse_network(nn.Module):
    ...
    def __init__(self, h_g, h_l, g, k, s, c):
        ...
        self.retina = retina(g, k, s) #(g=8, k=1, s=2)
    ...
    def forward(self, x, l_t_prev):
        ...
        # glimpse phi is the flatten vector of patch of x in location l_t_prev[i]
        ...
        phi_out = F.relu(self.fc1(phi)) # [32, 128]
        l_out = F.relu(self.fc2(l_t_prev)) # [32, 128]

        what = self.fc3(phi_out) # [32, 256]
        where = self.fc4(l_out) # [32, 256]

        # feed to fc layer
        g_t = F.relu(what + where) # [32, 256]
